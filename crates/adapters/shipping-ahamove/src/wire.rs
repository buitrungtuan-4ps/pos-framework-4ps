...
//!
//! [`CourierTransport`] is the one thing that touches a socket; [`TlsCourierTransport`] is the
//! concrete sender, on the rustls/hyper stack already in the tree
//! ([ADR-0038](../../../docs/adr/0038-webhook-tls-sender.md), [ADR-0054](../../../docs/adr/0054-edge-cloud-http-client.md)).
//! A courier's API base URL is operator configuration — one fixed, trusted host — so this dials the
//! ordinary way; there is no SSRF surface to defend, exactly as for the edge->cloud client.
//!
//! Unlike the edge->cloud channel, a courier is a REST resource: booking is a `POST`, cancelling is a
//! `POST` to the job's cancel sub-path, and tracking is a `GET` of the job. So the seam carries a
//! [`Method`] rather than fixing `POST`, and the body is optional. Keeping the seam to one method
//! keeps the stub the contract suite runs against trivial.
...
/// The HTTP verb a courier call uses.
///
/// A courier job is a REST resource: it is created (`POST`), read (`GET`), and cancelled (`POST` to a
/// sub-path). Naming the verb here keeps the mapping honest rather than tunnelling every call through
/// one `POST`.
...
pub trait CourierTransport: Send + Sync {
...
/// A response from the courier: the HTTP status and the body bytes.
#[derive(Debug, Clone)]
...
/// A transport-level failure — the courier could not be reached — as distinct from a response the
/// caller must interpret. Always maps to [`unavailable`](pos_ports::PortError::unavailable).
#[derive(Debug, thiserror::Error)]
#[error("the courier transport failed: {0}")]
...
#[derive(Debug, Clone)]
pub struct TlsCourierTransport {
    base: url::Url,
    tls: Arc<ClientConfig>,
    timeout: Duration,
}

impl TlsCourierTransport {
    /// Builds a transport pointed at `base_url` (the courier's API), trusting the bundled Mozilla
    ...
    pub fn new(base_url: &str, timeout: Duration) -> Result<Self, TransportError> {
        let base = url::Url::parse(base_url).map_err(|error| {
            TransportError::new(format!("the courier base URL is unparseable: {error}"))
        })?;
        if base.scheme() != "https" {
            return Err(TransportError::new("the courier base URL must use https"));
        }
        if base.host_str().is_none() {
            return Err(TransportError::new("the courier base URL has no host"));
        }
    ...
    async fn connect(host: &str, port: u16) -> Result<TcpStream, TransportError> {
        let addresses: Vec<SocketAddr> = tokio::net::lookup_host((host, port))
            .await
            .map_err(|error| {
                TransportError::new(format!("resolving the courier host failed: {error}"))
            })?
            .collect();
        ...
        Err(TransportError::new(match last_error {
            Some(error) => format!("connecting to the courier failed: {error}"),
            None => "the courier host resolved to no addresses".to_owned(),
        }))
    }
    ...
    async fn send(
        ...
        let server_name = ServerName::try_from(host.clone()).map_err(|_ignored| {
            TransportError::new("the courier host is not a valid TLS server name")
        })?;
...
impl CourierTransport for TlsCourierTransport {
    async fn request(
        &self,
        method: Method,
        path: &str,
        ...
        // A black-hole courier must not wedge the caller: the whole resolve→connect→handshake→send is
        ...
        match tokio::time::timeout(self.timeout, self.send(method, path, body)).await {
            Ok(result) => result,
            Err(_elapsed) => Err(TransportError::new("the request to the courier timed out")),
        }
    }
}
...
#[cfg(test)]
mod tests {
    use super::{Method, TlsCourierTransport};
    use core::time::Duration;
    ...
    fn a_plain_http_base_is_rejected() {
        let built = TlsCourierTransport::new("http://api.ahamove.example", Duration::from_secs(5));
        assert!(built.is_err(), "the courier base URL must be https");
    }

    #[test]
    fn an_unparseable_base_is_rejected() {
        let built = TlsCourierTransport::new("not a url", Duration::from_secs(5));
        assert!(built.is_err(), "a garbage base URL is a construction error");
    }

    #[test]
    fn a_valid_https_base_builds() {
        let built = TlsCourierTransport::new("https://api.ahamove.example", Duration::from_secs(5));
