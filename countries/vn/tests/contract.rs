...
//! The `Fiscalization` contract suite, against Vietnam's module.
//!
...
//! by [`pos_country::offline`], which this pack constructs with Vietnam's own number format
...
/// Supplies a fresh fiscalization for Vietnam.
///
...
impl FiscalizationHarness for VnHarness {
    type Fiscal = OfflineFiscalization;

    async fn fresh(&self) -> Setup<Self::Fiscal> {
        Ok(pos_country_vn::fiscalization())
    }
...
pos_contract_tests::fiscalization_suite!(VnHarness, run_ready);
