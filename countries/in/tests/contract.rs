...
//! The `Fiscalization` contract suite, against India's module.
//!
...
//! by [`pos_country::offline`], which this pack constructs with India's own number format
...
/// Supplies a fresh fiscalization for India.
///
...
impl FiscalizationHarness for InHarness {
    type Fiscal = OfflineFiscalization;

    async fn fresh(&self) -> Setup<Self::Fiscal> {
        Ok(pos_country_in::fiscalization())
    }
...
pos_contract_tests::fiscalization_suite!(InHarness, run_ready);
