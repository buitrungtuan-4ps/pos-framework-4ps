...
- **Courses** group lines (starter, main, dessert). Fire by course or fire everything. *Built so far ([ADR-0130](adr/0130-a-course-is-something-the-catalog-names.md)): a **course is a catalog entity** — tenant-scoped, named, carrying the `sort` that is the whole meaning of the grouping — authored through `/admin/catalog/courses`, and an item declares which course it goes out on the way it declares its tax class. Everything around it was already built: the event field, the line route, the station routing rule that matches on one and the edge that honours it, the `courses_enabled` capability and the domain's refusal when it is off. **Not yet done:** the compiled `menu` node does not carry the courses, so a store cannot read them; and nothing fires by course, though `LineCommand::Fire { course }` and its gate are written and tested. Each is its own change.*
