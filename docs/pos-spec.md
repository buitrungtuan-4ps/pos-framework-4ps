...
- **Courses** group lines (starter, main, dessert). Fire by course or fire everything. *Everything around a course is built — the event field, the line route, the station routing rule that matches on one, the `courses_enabled` capability and the domain's refusal when it is off — and the **entity itself does not exist**, so every `course_id` in the tree points at nothing. [ADR-0130](adr/0130-a-course-is-something-the-catalog-names.md) decides what a course is, where it is authored and how it reaches a store; nothing is implemented yet.*
