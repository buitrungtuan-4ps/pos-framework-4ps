...
| 0033 | The four-level Tenant→Brand→Store→Device config tree: layers deep-merge (most-specific wins), deltas are RFC 7386 JSON Merge Patch (deletion via `null`, diff/apply round-trips), a version is validated against §10 inter-flag rules before publish (rejection keeps last-good), and a store more than K=20 versions behind gets a full snapshot |
| 0034 | Super-admin auth: Argon2id password (same primitive as the edge PIN) plus a mandatory RFC 6238 TOTP second factor over HMAC-SHA256 (reuses `sha2`, single-use with a ±1-step skew window); both factors evaluated with no which-failed oracle; the session cookie is host-only (`__Host-`, no `Domain`) so it never crosses subdomains |
| 0035 | Retention/PII-masking cron masks the subject store (personal field values → `[REDACTED]`, keeping `subject_id`/timestamps so invoices still reconcile) rather than deleting rows; the period is configuration (country default), never a code guess; the sweep is bounded and idempotent; erasure/access requests stay escalated, and employee data is never touched |
| 0036 | Dashboards answer from a materialised rollup, not a per-view log scan: a projector cursor folds each event once (idempotent, rebuildable by resetting the cursor) with the same `fold_event` the from-log path uses, and the dashboard read takes no `EventStore` so it is O(days) and cannot scan the log — the P7 <10ms exit |
| 0037 | Scoped per-tenant API keys for machine access to `/v1`: `pos_<id>_<secret>` bearer tokens, only `SHA-256(secret)` stored (fast hash is right for a high-entropy token, unlike the Argon2 human password), constant-time verify, bound to one tenant (isolation) and deny-by-default by scope (least privilege), revocable, optionally expiring, and shown once |
