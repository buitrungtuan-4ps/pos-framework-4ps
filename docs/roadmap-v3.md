...
- **B2.2a** — *(delivered)* The QR staff-confirmation hold is real: derived from the log and the live policy, gating fire, with confirm/reject routes, the `sales.order.confirm_qr` permission and the till's Guests screen ([ADR-0116](adr/0116-the-qr-hold-is-derived-and-it-gates-firing.md)). `sales.qr_session.started` and `sales.order.submitted_by_guest` stay unproduced by design — ADR-0057 declined the per-session token they need for v1.
...
- **B2.5** — Modifiers/toppings on the dine-in path (additive line/event fields; min/max enforced from the modifier group; KDS shows them). *B1.1's proper home for the plumbing.* **Delivered** ([ADR-0127](adr/0127-modifier-groups-reach-the-edge.md)): the cloud compiles its modifier groups onto the `menu` node, the edge refuses a line that breaks an attached group's rule, and the till opens a picker for an item that has one. The **KDS and the pass show them too**, as does the order list, and `GET /api/orders/live` carries them so a screen that reloads or comes on mid-service rebuilds its tickets with the choices intact.
