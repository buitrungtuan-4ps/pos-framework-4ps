...
- **R2** — OTA artifact server + Garage store + promote-release; the cloud stays a dumb host, the edge verifies the signature ([ADR-0088](adr/0088-ota-artifact-hosting.md)). **The release registry landed (`pos_cloud::ota`, migration 0038); nothing reads it yet.** Remaining, in order: **move the route off `/internal`** — the proxy denies that prefix to anything off the box, so the path ADR-0054 pinned is unreachable by the store that calls it, and the handler becomes `POST /sync/stores/{store_id}/artifact` on the `read_config` scope ([ADR-0088](adr/0088-ota-artifact-hosting.md) Amendment 1); then artifact storage over `BlobStore` (adds `blob-garage` to `pos-cloud` plus S3 credentials the deployment does not provision — Correction 1), the route itself with the additive `arch` field the pinned request lacks (Correction 2), the adapter's bearer, then `/admin` upload + promote-release with audit and the runbook step.
