...
- **Q7** — UX step budget: a measured action budget for the common tasks, failable in CI (add item ≤2 taps, cash settle ≤3, price change ≤4 clicks). **Done** — `ui/scripts/step-budget.mjs` declares and resolves **fifteen** selling tasks (the sketch said "~12"), and `dashboard/scripts/step-budget.mjs` measures five office flows and reports rather than rules. Honest about the one case neither can see: a required tap nobody declared (gate register **A4**).
