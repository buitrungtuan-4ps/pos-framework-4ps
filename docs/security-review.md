...
| **P2** | On a headless Linux box the kernel keyring is not durable across a reboot. Answered by [ADR-0151](adr/0151-a-headless-linux-box-seals-its-secrets-with-systemd-creds.md): the edge seals its secrets under a vault key `systemd-creds` keeps (TPM2 when present, host key otherwise). Without a TPM2 a copied disk yields the credential; anyone running as `pos` or root on the live box can read it | Medium — availability, not disclosure; disclosure only from a copied disk of a box with no TPM2 | Built; hardware proof owed (gate P2) |
