...
| Integrity | Every event carries `seq` and the hash of the one before it, so an edited or deleted record breaks the chain. The store verifies its own chain with no internet, and publishes its head to the cloud at each shift close as `store.chain.anchored` ([ADR-0131](adr/0131-a-chained-event-log.md)). The cloud files every anchor, one head per chain length per store, and **refuses one that contradicts a head it already holds** — which closes **recomputation**. It then **recomputes the chain from its own copies** of the events that anchor covers ([ADR-0132](adr/0132-the-cloud-recomputes-the-chain-it-holds.md)), which closes **truncation**: a store cut back and then traded on anchors *above* anything the cloud holds and looks like honest growth, but leaves two different events claiming one `chain.seq`, and the events say what the anchors cannot. A missing record is reported as *incomplete* and never as tampering — a broker gap is ordinary, and [ADR-0040](adr/0040-reconciliation.md) exists to fill it. The guarantee reaches back to the **last anchor**, not the last event. Every refusal is listed on the console's **Chain integrity** screen (`GET /admin/chain-findings`), because a mechanism whose product is being noticed is worth little if nobody can see what it noticed. Tamper-**evident**, never tamper-proof: it makes what somebody did visible, it does not stop them. |
...
| `SubjectStore` | Where personal data lives, keyed by `SubjectId`, so the immutable log never holds any | SQLite (edge-local) |
...
The list above is authoritative and counts **twenty** ports. [ADR-0006](adr/0006-ports-and-adapters.md) named fifteen: it omitted `OrderIn`, which [ADR-0012](adr/0012-qr-ordering-via-cloud.md) depends on, and [ADR-0021](adr/0021-corrected-port-list.md) supersedes it with sixteen. [ADR-0053](adr/0053-cloud-sync-port.md) adds the seventeenth, `CloudSync`, for the store↔cloud calls that need an answer back — distinct from `MessageLink`, which stays outbound-only so the store still sells offline. [ADR-0091](adr/0091-durable-edge-auth-state.md) adds the eighteenth, `DeviceRegistry`, so that pairing and sign-in survive a restart instead of being erased by one; it stores a SHA-256 of each device token and never the token. The nineteenth, `IntakeLedger` ([ADR-0064](adr/0064-edge-order-in.md)), is a port that was named as one when it landed and registered late — see below. The twentieth, `SubjectStore` ([ADR-0107](adr/0107-the-buyer-is-a-subject.md)), is the place a **store** keeps personal data: `pos_proto::pii` makes a name in an event a compile error and [`pos-spec.md`](pos-spec.md) §15 says why, so a B2B invoice's buyer is filed here — in the settle's own transaction — and the event carries only the subject id. It has no read-them-all, because a port that can enumerate personal data is a port that can export it.

**A discrepancy that was recorded rather than papered over, and is now closed.** `IntakeLedger` ([ADR-0064](adr/0064-edge-order-in.md)) spent two slices as a trait in `pos-ports` that its own record called a port while having no `PortName` variant, no contract suite, and no row in the table above; it reported its failures under `PortName::OrderIn`. It escaped the "every port has a suite" gate because that gate iterates `PortName::ALL`, so a trait added without a variant is invisible to it. It now has the variant, a six-case suite both `pos-fakes` and `store-sqlite` pass, a row above, and its own error label.

**The blind spot itself is still there**, and worth stating plainly: the gate can only check ports that were registered, so it can never be the thing that catches an *unregistered* one. What catches that is the rule that a new port needs an ADR ([ADR-0021](adr/0021-corrected-port-list.md)) and a reviewer who reads it — which is a process control, not a test, and it is the one that failed here.
