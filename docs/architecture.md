...
| Integrity | Every event carries `seq` and the hash of the one before it, so an edited or deleted record breaks the chain. The store verifies its own chain with no internet. Publishing the head to the cloud each shift close — what closes truncation and recomputation, neither of which a chain catches alone — is the half still to come ([ADR-0131](adr/0131-a-chained-event-log.md)). Tamper-**evident**, never tamper-proof: it makes what somebody did visible, it does not stop them. |
