...
| Integrity | Every event carries `seq` and the hash of the one before it, so an edited or deleted record breaks the chain. The store verifies its own chain with no internet, and publishes its head to the cloud at each shift close as `store.chain.anchored` ([ADR-0131](adr/0131-a-chained-event-log.md)). The cloud files every anchor it receives, one head per chain length per store, and **refuses one that contradicts a head it already holds at that length** — recording the contradiction rather than overwriting the record that shows it. That closes **recomputation**: a store that rewrites its history and re-derives the chain arrives at a different head for a length the cloud already has. It does **not** close **truncation** on its own — a store cut back and then traded on anchors *above* anything the cloud holds, which reads as honest growth; what gives that away is two different events claiming one `chain.seq`, a property of the event log rather than of the anchors, and not yet built. The guarantee reaches back to the **last anchor**, not the last event. Tamper-**evident**, never tamper-proof: it makes what somebody did visible, it does not stop them. |
