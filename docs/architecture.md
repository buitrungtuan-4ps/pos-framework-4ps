...
| Integrity | Every event carries `seq` and the hash of the one before it, so an edited or deleted record breaks the chain. The store verifies its own chain with no internet, and publishes its head to the cloud at each shift close as `store.chain.anchored` — a store cannot rewrite what the cloud has already received, which is what closes truncation and recomputation, neither of which a chain catches alone ([ADR-0131](adr/0131-a-chained-event-log.md)). The guarantee reaches back to the **last anchor**, not the last event. The cloud storing anchors and refusing one that does not extend the last is still to come. Tamper-**evident**, never tamper-proof: it makes what somebody did visible, it does not stop them. |
