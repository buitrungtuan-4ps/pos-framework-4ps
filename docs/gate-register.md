...
| A4 | **Counting the clicks a flow actually takes** | Both step budgets (`ui/scripts/step-budget.mjs`, `dashboard/scripts/step-budget.mjs`) resolve every *declared* tap against the source, so a renamed or deleted handler fails a pull request. What neither can see is a tap **nobody declared**: add a confirmation dialog and leave the declaration alone, and the gate stays green while the flow is a click worse. Closing it needs a browser driving the built app and counting real clicks — Playwright is already installed in this environment, so the missing pieces are a seeded edge (or a stubbed API) and the fixtures, not a new dependency. Until then a reviewer owns "did this change add a step?", and the printed map is what makes that a five-second question | `ui/` and `dashboard/` CI jobs, alongside the two analysers · [`ui-ux.md`](ui-ux.md) §1.6 |
