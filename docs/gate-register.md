...
| L3 | **Clear any cross-border transfer** — a DTA or explicit consent — before data leaves the country of collection | Any hosting region outside it. The hosting-region decision itself (APPI/DPDP) is B10.4 | [ADR-0076](adr/0076-subject-request-tooling.md) · [ADR-0114](adr/0114-region-is-required-recorded-visible.md) · [`roadmap-v3.md`](roadmap-v3.md) B10.4 |
| L3a | **Record the basis** for each store whose data rests outside its own country | A `differs` verdict on the store hub. The console warns and never blocks; the answer is a sentence a person writes, stored with both country codes and audited under `store.edge_placement.acknowledge`. The framework supplies the place to record it and decides nothing | [ADR-0114](adr/0114-region-is-required-recorded-visible.md) |
...
| P2 | **Headless-Linux credential durability across a reboot, on real hardware.** Built ([ADR-0151](adr/0151-a-headless-linux-box-seals-its-secrets-with-systemd-creds.md)): a vault key sealed with `systemd-creds` (TPM2 when present, host key otherwise), unsealed by root at each start, and the edge's secrets sealed under it in `/var/lib/pos-edge/vault`. Proven in a container against a real `systemd-creds` and real files; what a gate cannot do is **power-cycle a box with and without a TPM2** and see it come back activated | A headless Linux box surviving a power cycle without re-activation, proven on the boxes the fleet buys | [ADR-0086](adr/0086-edge-keyvault-and-activation.md) · [ADR-0151](adr/0151-a-headless-linux-box-seals-its-secrets-with-systemd-creds.md) |
...
| P11 | **That a headless Windows store produces a readable log, and that a code read out of it pairs a device.** The mechanism is built and unit-tested on Linux ([ADR-0117](adr/0117-a-headless-store-keeps-a-log.md)): the file opens, rotates, caps, refuses the pairing and auth targets, and survives a first sink that errors on every write — which is what a service's stdout handle does. What no gate here can run is the thing the record is about. Whether `POS_EDGE_LOG_FILE` and `POS_EDGE_PAIRING_FILE`, set as `REG_MULTI_SZ` on the service key, reach the process at all; whether `LocalSystem` can create `C:\ProgramData\pos-edge` and write in it; whether the installer's bounded wait is long enough on a shop-floor PC; and whether the URL the file holds actually redeems on a tablet. That needs a Windows machine with the service installed on it, and PowerShell cannot even be parsed on the `ubuntu-24.04` legs — the `build (windows-2022)` job is the only thing in CI that reads those scripts | **Windows as a pairable store target at all.** Until this is cleared, no Windows store can admit a device: the code is minted once per process start, the announcement goes to a discarded stdout, and no route mints a second one. This is the narrowest, highest-value hardware check in this section | [ADR-0117](adr/0117-a-headless-store-keeps-a-log.md) · [`deploy/edge/README.md`](../deploy/edge/README.md) · [`guides/bring-a-store-online.md`](guides/bring-a-store-online.md) |
| P12 | **The state directory's access control, on both platforms.** `C:\ProgramData\pos-edge` has **no explicit ACL** and `store.sqlite` sits in it under the inherited one, holding device token digests, staff sessions and subject rows permanently. ADR-0117 decision 9 declines to fix this with an `icacls` and says why: the state directory is a script *parameter*, so an elevated `icacls $Root /inheritance:r` with a wrong value is a destructive ACL-reset primitive, and `/inheritance:r` would leave SYSTEM and Administrators only — locking a standard-user till out of the log the installer's own text promises it can read. What is needed is a decision taken with a real machine's account layout in front of whoever takes it: which principal the service runs as, which reads the log, and which must never read the database. Pre-existing and larger than anything ADR-0117 introduced, which is why it is recorded rather than closed | Any claim that a store box's state is protected from a local user. It does not block bring-up | [ADR-0117](adr/0117-a-headless-store-keeps-a-log.md) decision 9 · [ADR-0091](adr/0091-durable-edge-auth-state.md) · [`security-review.md`](security-review.md) |
