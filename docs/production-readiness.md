...
| C2 | Every settle sets `print_receipt` and the till renders "Printing receipt…", but no binary depends on `printer-escpos` and nothing constructs a `PrintJob`. **Verified**; framed by [ADR-0100](adr/0100-receipt-and-ticket-printing.md) and sequenced as four slices: (1) the `pos-proto` `devices` node, (2a) approval captures a device's connection and station — which a discovered proposal cannot know, (2b) the cloud compiles and publishes the node, and (3) the edge applies it are **landed**; (4) the dispatcher that prints, with `backup_station_id` failover, is what remains. Real hardware stays in the §6 gate | **verified** (4 of 5 slices landed) |
