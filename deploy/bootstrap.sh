...
if [ ! -e "$SECRETS/cloud.toml" ]; then
  ...
  ARCHIVE_KEY_SECRET="$(rand_hex 32)"
...
# The secret every store's archive key is wrapped under before it is written to the database
# (ADR-0124 Amendment 1). Exactly 64 hex characters — it is a 32-byte key, not a passphrase, and
# pos_cloud refuses to start on a malformed one.
#
# It lives HERE and nowhere else, and that is the whole point: backup.sh ships a pg_dump of the
# cloud database off-box with rclone, to the same tier the sealed store archives sync to. This
# file is not in that dump, so the archives travel as ciphertext the archive tier cannot open.
# Losing this value orphans every key already wrapped under it, so it is minted once and never
# rotated — the same rule as every other secret in this file.
archive_key_secret = "$ARCHIVE_KEY_SECRET"
...
EOF
  ...
  # archive_key_secret is minted IF ABSENT on the same terms. Unlike internal_shared_secret its
  # absence is not fatal — pos_cloud boots and turns store archives off, saying so — which is
  # exactly why it needs a reconcile: a box upgraded across ADR-0124 would otherwise run happily
  # with no store backing its database up, and the only clue would be a warn line on boot.
  archive_note='# Secret every store archive key is wrapped under (ADR-0124), added by a later bootstrap run.'
  archive_rc=0
  cloud_toml_mint_key_if_absent archive_key_secret "$archive_note" || archive_rc=$?
  case "$archive_rc" in
    0) echo "set    cloud.toml archive_key_secret (store archives were off on this box)" ;;
    1) ;;
    2) echo "warn   could not read $SECRETS/cloud.toml (need root or passwordless sudo); cannot tell whether archive_key_secret is set" ;;
    *) echo "warn   could not write archive_key_secret to $SECRETS/cloud.toml (need root or passwordless sudo); store archives stay off" ;;
  esac
